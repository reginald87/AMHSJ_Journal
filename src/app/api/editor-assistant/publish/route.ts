...
const logger = createLogger('EditorAssistantPublish');
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!manuscriptId || !volumeId) {
      return NextResponse.json({ error: 'Manuscript and volume are required' }, { status: 400 });
    }
    ...
    if (manuscript.status !== 'ACCEPTED') {
      return NextResponse.json({ error: 'Only accepted manuscripts can be published' }, { status: 400 });
    }
    ...
    const article = await prisma.$transaction(async (tx) => {
      ...
      await tx.articleMetric.create({
        data: { articleId: createdArticle.id, views: 0, downloads: 0, citations: 0, shares: 0, mentions: 0 },
      });

...
export default async function FAQPage() {
  ...
  const faqs = s?.faqs ?? DEFAULTS.faqs;
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-navy-950">
      ...
      <section className="gradient-navy relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 sm:py-32">
          <div className="max-w-3xl animate-fade-in">
            <span className="inline-flex items-center gap-2 px-4 py-2 bg-gold-500/20 text-gold-400 rounded-full text-sm font-medium mb-6">Help Center</span>
      ...
      <section className="py-8 bg-white dark:bg-navy-900 border-b border-slate-200 dark:border-navy-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-wrap gap-3 justify-center text-sm">
            {faqs.map((cat: { category: string }) => (
              <a key={cat.category} href={`#${cat.category.toLowerCase()}`} className="px-4 py-2 rounded-lg bg-slate-100 dark:bg-navy-800 text-slate-700 dark:text-slate-300 hover:bg-navy-100 dark:hover:bg-navy-700 transition-colors">{cat.category}</a>
            ))}
          </div>
        </div>
      </section>
      ...
      {/* FAQ Sections */}
      {faqs.map((cat: { category: string; items: { q: string; a: string }[] }, ci: number) => (
        <section key={cat.category} id={cat.category.toLowerCase()} className={`py-16 ${ci % 2 === 1 ? 'bg-white dark:bg-navy-900 border-t border-slate-200 dark:border-navy-800' : ''}`}>
          <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <Badge variant="navy" size="lg" className="px-4 py-2 text-sm mb-6">{cat.category}</Badge>
            <div className="space-y-4">
              {cat.items.map((faq: { q: string; a: string }, fi: number) => (
                <details key={fi} className="group bg-white dark:bg-navy-900 rounded-xl border border-slate-200 dark:border-navy-800 overflow-hidden">
                  ...
                  <div className="px-6 pb-4 text-sm text-slate-600 dark:text-slate-400 leading-relaxed border-t border-slate-100 dark:border-navy-800 pt-4">{faq.a}</div>
                </details>
              ))}
            </div>
          </div>

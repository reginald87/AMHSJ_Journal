...
async function getSections() {
  const page = await prisma.page.findUnique({ where: { slug: 'ethics' } });
...
export default async function EthicsPage() {
  ...
  const principles = (s?.principles ?? DEFAULTS.principles).map((p: { icon: string; title: string; items: string[] }) => ({
    ...p,
    icon: ICON_MAP[p.icon] || Shield,
  }));
  const misconduct = s?.misconduct ?? DEFAULTS.misconduct;
  const process = s?.process ?? DEFAULTS.process;
      ...
      <section className="py-8 bg-white dark:bg-navy-900 border-b border-slate-200 dark:border-navy-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <p className="text-sm text-slate-600 dark:text-slate-400">
            ...
            <a href="https://publicationethics.org" target="_blank" rel="noopener noreferrer" className="text-navy-700 dark:text-gold-400 font-semibold hover:underline">Committee on Publication Ethics (COPE)</a>
      ...
      <section className="py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <h2 className="text-3xl sm:text-4xl font-bold text-navy-900 dark:text-white mb-4">Ethical Principles</h2>
            <p className="text-slate-600 dark:text-slate-400 max-w-2xl mx-auto">Our ethical standards apply to all participants in the publishing process: authors, reviewers, and editors.</p>
          </div>
          <div className="grid md:grid-cols-2 gap-6 max-w-5xl mx-auto">
            {principles.map((section: { icon: React.ComponentType<{ className?: string }>; title: string; items: string[] }, i: number) => {
      ...
      <section className="py-20 bg-white dark:bg-navy-900 border-t border-slate-200 dark:border-navy-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <h2 className="text-3xl sm:text-4xl font-bold text-navy-900 dark:text-white mb-4">Research Misconduct</h2>
            <p className="text-slate-600 dark:text-slate-400 max-w-2xl mx-auto">AMHSJ maintains a strict zero-tolerance policy toward research misconduct.</p>
          </div>
          <div className="grid md:grid-cols-2 gap-6 max-w-5xl mx-auto">
            {misconduct.map((item: { title: string; description: string; severity: string }, i: number) => (
              <div key={i} className="bg-slate-50 dark:bg-navy-950 rounded-xl p-6 border border-slate-200 dark:border-navy-800">
                ...
                <p className="text-xs font-medium text-red-700 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-3 py-1.5 rounded-lg inline-block">Consequence: {item.severity}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <h2 className="text-3xl sm:text-4xl font-bold text-navy-900 dark:text-white mb-4">Ethics Complaint Process</h2>
          </div>
          <div className="max-w-3xl mx-auto">
            <div className="relative">
              <div className="absolute left-6 top-0 bottom-0 w-0.5 bg-navy-200 dark:bg-navy-700" />
              <div className="space-y-6">
                {process.map((item: { step: string; description: string }, i: number) => (
                  <div key={i} className="relative flex gap-6">
                    <div className="w-12 h-12 bg-navy-900 dark:bg-navy-700 text-gold-400 rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0 z-10">{i + 1}</div>
